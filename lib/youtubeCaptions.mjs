...
async function fetchWatchHtml(videoId) {
  ...
  try {
    const watchResponse = await fetch(watchUrl, { headers: WATCH_HEADERS, signal: timeoutSignal(10000) });
...
async function downloadCaptionTrack(track, includeTimestamps, fetchOptions = {}) {
  for (const url of captionDownloadUrls(track.baseUrl || track.url)) {
    try {
      const response = await fetch(url, { headers: WATCH_HEADERS, signal: timeoutSignal(15000), ...fetchOptions });
  ...
  for (const lang of langs) {
    for (const fmt of ['json3', 'srv3', 'vtt', '']) {
      try {
        ...
        const response = await fetch(url, { headers: WATCH_HEADERS, signal: timeoutSignal(10000), ...fetchOptions });
        if (!response.ok) continue;
        const text = await response.text();
        if (!text?.trim()) continue;
