...
async function assertUserAndCredits(supabase, user) {
  ...
  if (!trialActive) {
    ...
    if ((count || 0) >= 1) {
      const error = new Error(
        'Your free daily credit is used. The unlimited 7-day offer is available for new accounts.',
      );
      error.status = 402;
      throw error;
    }
  ...
  return { trialEndsAt, trialActive };
}
...
async function fetchTranscriptText({
  ...
  const direct = await fetchTranscriptFromYouTubeCaptions(videoId, {
    language,
    includeTimestamps,
  });
...
export async function generateTranscript({
  ...
  const { trialEndsAt, trialActive } = await assertUserAndCredits(supabase, user);
